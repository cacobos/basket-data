...
let FebService = class FebService {
    ...
    async extractMatchIdsFromCalendars(calendarUrls, teamId) {
        ...
        for (const calendarUrl of calendarUrls) {
            try {
                ...
                console.log(...oo_oo(`102363957_206_8_208_9_4`, `[extractMatchIdsFromCalendars] Filtering calendar for team ${normalizedTeamId}...`));
                const $ = (0, cheerio_1.load)(html);
                ...
                console.log(...oo_oo(`102363957_231_8_233_9_4`, `[extractMatchIdsFromCalendars] Found ${foundByTeamFilter} matches for team ${normalizedTeamId} (total extracted: ${ids.size})`));
            }
            catch (error) {
                console.error(...oo_tx(`102363957_237_8_240_9_11`, `[extractMatchIdsFromCalendars] Error processing calendar ${calendarUrl}:`, error));
            }
        }
        return Array.from(ids);
    }
    ...
    async getActionPlayers(teamId, matchIds) {
        ...
        for (const matchId of normalizedMatchIds) {
            if (!/^\d{6,}$/.test(matchId)) {
                console.log(...oo_oo(`102363957_373_8_373_76_4`, `[getActionPlayers] Skipping mock match ID: ${matchId}`));
                continue;
            }
        ...
        if (appearances.size === 0) {
            console.log(...oo_oo(`102363957_397_6_399_7_4`, `[getActionPlayers] No action data found in play-by-play for team ${normalizedTeamId}. Returning empty array.`));
            players = [];
        }
        else {
    ...
    async getTeamMatches(teamId) {
        ...
        console.log(...oo_oo(`102363957_433_4_435_5_4`, `[getTeamMatches] Fetching matches for team: ${normalizedTeamId}, URL: ${teamUrl}`));
        let response = await fetch(teamUrl);
        if (!response.ok) {
            response = await fetch(legacyTeamUrl);
        }
        if (!response.ok) {
            console.error(...oo_tx(`102363957_443_6_445_7_11`, `[getTeamMatches] Failed to fetch team page: ${response.status}`));
        ...
        if (matchIds.length === 0) {
            ...
            for (const league of leagues) {
                try {
                    ...
                    console.log(...oo_oo(`102363957_471_10_473_11_4`, `[getTeamMatches] Team ${normalizedTeamId} found in league ${league.slug}. Calling extractMatchIdsFromCalendars()...`));
                    ...
                    console.error(...oo_tx(`102363957_483_10_486_11_11`, `[getTeamMatches] Error querying league ${league.slug}:`, error));
                }
            }
        }
        matchIds = Array.from(new Set(matchIds));
        console.log(...oo_oo(`102363957_492_4_494_5_4`, `[getTeamMatches] Starting play-by-play validation for ${matchIds.length} matchIds...`));
        const scrapUtils = await this.loadScrapUtilsModule();
        const matches = [];
        for (const matchId of matchIds) {
            try {
                ...
                if (!Array.isArray(lines) || lines.length === 0) {
                    console.log(...oo_oo(`102363957_506_10_508_11_4`, `[getTeamMatches] No play-by-play data for match ${matchId}. Skipping.`));
                    continue;
                }
                let team1Id = null;
                let team2Id = null;
                ...
                if (!team1Id || !team2Id) {
                    console.log(...oo_oo(`102363957_544_10_546_11_4`, `[getTeamMatches] Match ${matchId}: missing team IDs. Skipping.`));
                    continue;
                }
                if (team1Id !== normalizedTeamId && team2Id !== normalizedTeamId) {
                    console.log(...oo_oo(`102363957_552_10_554_11_4`, `[getTeamMatches] Match ${matchId}: team ${normalizedTeamId} not in (${team1Id} vs ${team2Id}). Skipping.`));
                    continue;
                }
                ...
                console.error(...oo_tx(`102363957_589_8_592_9_11`, `[getTeamMatches] Exception processing match ${matchId}:`, error));
                continue;
            }
        }
        if (matches.length === 0) {
            console.log(...oo_oo(`102363957_598_6_600_7_4`, `[getTeamMatches] No valid matches found for team ${normalizedTeamId}. Returning empty list.`));
            return {
                teamId: normalizedTeamId,
                matches: [],
            };
        }
        console.log(...oo_oo(`102363957_607_4_609_5_4`, `[getTeamMatches] Completed. Found ${matches.length} valid matches for team ${normalizedTeamId}.`));
        return {
            teamId: normalizedTeamId,
