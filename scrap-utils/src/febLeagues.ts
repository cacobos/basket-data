...
function buildLeaguePageUrl(slug: string, leagueId: string, seasonId: string): string {
  return `${FEB_BASE_URL}/estadisticas/${slug}/${leagueId}/${seasonId}`;
}

function parseLeaguesFromHome(html: string): FebLeague[] {
  const regex = /https:\/\/baloncestoenvivo\.feb\.es\/estadisticas\.aspx\?g=(\d+)&(?:amp;)?t=(\d+)&(?:amp;)?nm=([a-z0-9]+)/gi;
  const unique = new Map<string, FebLeague>();
...
function readAspNetFormState(html: string, seasonId: string): AspNetFormState {
  const $ = load(html);
...
function parseGroups(html: string): FebLeagueGroup[] {
  const $ = load(html);
  const groupSelect = $("#_ctl0_MainContentPlaceHolderMaster_fasesGruposDropDownList");
...
function parseTeams(html: string): FebLeagueTeam[] {
  const $ = load(html);
  const teamMap = new Map<string, FebLeagueTeam>();
...
async function fetchLeaguePage(slug: string, leagueId: string, seasonId: string): Promise<string> {
...
async function postLeagueGroup(html: string, seasonId: string, groupId: string): Promise<string> {
...
export async function getFebLeagues(): Promise<FebLeague[]> {
...
export async function getFebLeagueTeams(
